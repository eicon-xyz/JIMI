...
## [2026-07-01] RTX 5070 Ti / sm_120 本地 OmniParser 修复

- **`scripts/detect_omni_device.py`**：实测 CUDA kernel，sm_120 不在 PyTorch arch list 时强制 CPU
- **`scripts/start_omniparser.bat`**：不再仅用 `cuda.is_available()`；支持 `OMNI_FORCE_CPU` / `OMNI_FORCE_CUDA`
- **`scripts/start_local_demo.bat`**：本地演示一键 CPU；探测校园 GPU 并提示切换内网
- **`scripts/upgrade_omni_pytorch_cu128.bat`**：可选本机 RTX 50 GPU 升级路径
- **`core/deployment_resolver.py`**：启动时探测校园 A 端并给出 UI 提示
- **B 端**：`main.py` / `api_client.py` 状态文案区分「校园 GPU/cuda」与「本地 CPU」

---

## [2026-07-01] auto 检测链 + health 扩展

- **`DETECTOR_BACKEND=auto`**：探测顺序 `OMNIPARSER_GPU_URL` → `OMNIPARSER_LOCAL_URL` → Replicate（可选）
- **`/api/demo/health` 新字段**：`detector_active`、`detector_device`、`omniparser_url`（保留 `detector_backend`、`omniparser_ready`）
- **OmniParser `/probe/`**：返回 `device`、`cuda_available`
- **B 端联动**：系统设置保存时合并写入 `server/.env`；内网模式 B 只连远程 A
- **部署 runbook**：[`A端-学校GPU部署与联调指南_v2.md`](A端-学校GPU部署与联调指南_v2.md)
- 涉及：`server/services/ui_detector.py`, `server/config.py`, `server/routes/demo.py`, `server/models/schemas.py`, `server/.env.example`
...
**RTX 50 系注意**：PyTorch cu124 不支持 sm_120。`scripts/detect_omni_device.py` + `start_omniparser.bat` 会实测 CUDA kernel，不支持时强制 **CPU**（~2–4 分钟/帧）。校园 GPU 优先内网 API；本机 GPU 可选 `scripts/upgrade_omni_pytorch_cu128.bat`。

**启动顺序**：① `start_omniparser.bat`（或 `start_local_demo.bat`）→ ② `start_server.bat` → ③ B 端 `python main.py`
